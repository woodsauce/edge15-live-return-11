...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-23.zip';
...
function hydrateSettings() {
  ...
  ['useCurrentAsTarget', 'start15m'].forEach((id) => {
    const btn = $(id);
    if (btn) { btn.disabled = true; btn.title = 'Manual target/time disabled in v22. Official KXBTC15M data required.'; }
  });
}
...
function getTargetAuthorityStatus(now = Date.now()) {
  ...
  return { official: false, source: 'missing_official', label: 'Waiting for official KXBTC15M target', className: 'pill bad', reason: 'No verified official target available. v21 blocks official locks and scoring until data is verified.' };
}
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const underOpportunityClean = decision.action !== 'UNDER' || (
    rightSide &&
    chartSupport.available && chartSupport.supports &&
  ...
  const sixMinuteMoneyLock = remainingSec <= 420 && remainingSec >= GENESIS_MONEY_LATEST_SEC && baseClean && entryQuality >= 52 && rightSideOrEliteReclaim && cushion.score >= sixMinuteCushionRequired && genesisClean && overSixClean && underOpportunityClean;
  const tenMinuteElite = minute === 10 && confidence >= 92 && stability >= 90 && flipRisk <= 48 && entryQuality >= 76 && heldMs >= 30000 && rightSide && cushion.score >= eliteCushionRequired && genesisElite && overEarlyClean && underOpportunityClean;
  const earlyGenesisLock = remainingSec <= 540 && remainingSec > 420 && confidence >= 62 && flipRisk <= 52 && entryQuality >= 70 && heldMs >= 30000 && rightSide && cushion.score >= earlyCushionRequired && genesisElite && overEarlyClean && underOpportunityClean;
  ...
  const emergency = remainingSec >= GENESIS_MONEY_LATEST_SEC && confidence >= 90 && flipRisk <= 28 && entryQuality >= 84 && rightSide && cushion.score >= emergencyCushionRequired && genesisElite && overEarlyClean && underOpportunityClean && emergencyClean;
...
async function loadCoinbasePrediction(silent = false) {
  ...
  try {
    ...
    const debug = $('apiDebug');
    if (debug) {
      debug.textContent = `Official target fetch failed: ${error.message}. v22 will not create a local target, tracker lock, money guidance, cash-out alert, or win/loss score until KXBTC15M data is verified.`;
    }
  }
}
...
function getTradingStyleRecommendation(decision = state.decision, candidate = null) {
  ...
  return {
    styleKey,
    styleLabel: style.label,
    status: balancedOk ? 'Balanced money OK' : (thinBalancedPayout || expensiveWarning) ? 'Lock valid · low payout' : 'Wait',
    level: balancedOk ? 'good' : (thinBalancedPayout || expensiveWarning) ? 'warn' : 'quiet',
    message: (balancedOk
      ? `Balanced: tracker lock valid. ${payoutText}.`
      : (thinBalancedPayout || expensiveWarning)
        ? `Balanced: tracker lock can still be valid, but payout is thin/contract expensive. ${payoutText}.`
        : `Balanced: waiting for a valid tracker setup.`) + targetWarning,
    note: style.note
  };
}
...
function mergeRecords(records = []) {
  ...
  const add = (record) => {
    ...
    if (!existing || Number(record.settledAt || record.ts || 0) >= Number(existing.settledAt || existing.ts || 0)) map.set(key, record);
  };
...
async function pushCloudTracker({ silent = false } = {}) {
  try {
    const officialRecords = state.records.filter((record) => record?.ticker && String(record.ticker).startsWith('KXBTC15M-'));
...
function setTargetFromCurrent() {
  $('readiness').textContent = 'Manual target disabled in v21. Waiting for verified KXBTC15M target.';
}

function startManualCountdown(minutes = 15) {
  $('readiness').textContent = 'Manual countdown disabled in v21. Waiting for verified official market close time.';
}
