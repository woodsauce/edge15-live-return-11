...
test('chart signal layer reads rising pressure as OVER', () => {
  const sample = ticks(100000, 18, 120);
  const chart = analyzeChartSignals({
    ticks: sample,
    price: sample.at(-1).price,
    targetPrice: 100900,
    remainingSec: 390
  });
  assert.equal(chart.available, true);
  assert.ok(chart.overScore > chart.underScore);
  assert.ok(['OVER', 'NEUTRAL'].includes(chart.direction));
});

test('evaluateDecision exposes chart signal layer for UI and Genesis weighting', () => {
  const decision = evaluateDecision({
    ticks: ticks(100000, 14, 100),
    targetPrice: 100550,
    timeRemainingSec: 390,
    profile: 'balanced',
    recentDecisions: [{ choice: 'OVER', confidence: 70 }]
  });
  assert.equal(decision.chartSignals.available, true);
  assert.ok(Number.isFinite(decision.chartSignals.overScore));
  assert.ok(decision.reasons.some((reason) => reason.includes('Chart Signal Layer')));
});
