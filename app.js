...
const APP_VERSION_FILE = 'edge15-genesis-elite-25.zip';
...
const SCOUT_MIN_GENESIS_SUCCESS = 70;
const SCOUT_MAX_GENESIS_RISK = 50;
...
const ELITE_MAX_FLIP_RISK = { OVER: 38, UNDER: 40 };
const ELITE_MIN_CHART_SCORE = { OVER: 72, UNDER: 68 };
const ELITE_MIN_SUCCESS_CHANCE = { OVER: 72, UNDER: 70 };
const ELITE_STRONG_CUSHION_SCORE = 115;
const ELITE_MAX_ODDS_AGE_SEC = 20;
const DISABLED_MONEY_LOCKS = ['10m genesis elite lock'];
const NO_ENTRY_LOCKS = ['late proof lock · no-entry'];
const SCOUT_MIN_STABILITY = 80;
const SCOUT_MIN_CHART_BY_SIDE = { OVER: 74, UNDER: 70 };
const SCOUT_MAX_FLIP_BY_SIDE = { OVER: 38, UNDER: 40 };
...
function getEliteAccuracyGuard(decision, candidate = {}, source = '') {
  ...
  if (DISABLED_MONEY_LOCKS.some((item) => normalizedSource.includes(item))) {
    blockers.push('10m Genesis elite lock is preview-only until it proves 90%+ again.');
  }
  ...
  return { pass: blockers.length === 0, blockers, warnings, chartScore, successChance, stability, flipRisk, entryQuality };
}
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const gradeClean = !['C', 'D', 'F'].includes(String(genesisGate.tradeGrade || '').toUpperCase());
  const strongCushion = Number(cushion.score || 0) >= ELITE_STRONG_CUSHION_SCORE;
  const commonStrict = baseClean && entryQuality >= ELITE_MIN_ENTRY_QUALITY && rightSideOnly && strongCushion && chartClean && genesisClean && gradeClean && cushionVelocity >= -2;
  ...
  const emergencyClean = confidence >= 92 && flipRisk <= 28 && entryQuality >= 86 && rightSide && Number(cushion.score || 0) >= emergencyCushionRequired && chartClean && genesisClean && gradeClean && Number(chartSupport.targetCrosses || 0) <= 2 && cushionVelocity >= 0;
  const emergency = remainingSec >= GENESIS_MONEY_LATEST_SEC && emergencyClean;
