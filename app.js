...
const APP_VERSION_FILE = 'edge15-genesis-model-tournament-34-multi-model-lab.zip';
const LEARNING_SAMPLE_TARGET = 25;
const STORE_KEY = 'edge15.clone.tournament34.records.v1';
const SETTINGS_KEY = 'edge15.clone.tournament34.settings.v1';
const LADDERS_KEY = 'edge15.clone.tournament34.ladders.v1';
const CURRENT_LADDER_KEY = 'edge15.clone.tournament34.currentLadder.v1';
const ODDS_CACHE_KEY = 'edge15.clone.tournament34.oddsCache.v1';
const MAX_TICKS = 900;
...
const TOURNAMENT_RECORDS_KEY = 'edge15.clone.tournament34.modelRecords.v1';
const TRACKER_LOCK_VERSION = 34;
...
function evaluateTournamentModels(decision = state.decision, remainingSec = getRemainingSec()) {
  ...
  for (const model of TOURNAMENT_MODEL_DEFS.filter((item) => item.id !== 'meta_judge')) {
    const result = getTournamentModelDecision(model, snapshot, decisions);
    decisions.push({ ...result, modelId: model.id, label: model.label, family: model.family });
  }
  const metaModel = TOURNAMENT_MODEL_DEFS.find((item) => item.id === 'meta_judge');
  if (metaModel) {
    const result = getTournamentModelDecision(metaModel, snapshot, decisions);
    decisions.push({ ...result, modelId: metaModel.id, label: metaModel.label, family: metaModel.family });
  }
  return decisions;
}
...
function addTournamentRecordsFromLadder(ladder) {
  ...
  for (const model of TOURNAMENT_MODEL_DEFS) {
    ...
    const lock = (ladder.modelLocks || {})[model.id] || null;
    newRecords.push({
