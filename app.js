...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-17.zip';
...
function getTargetAuthorityStatus(now = Date.now()) {
  ...
  const isCoinbase = prediction?.source === 'coinbase_predictions' && Number.isFinite(Number(prediction?.targetPrice));
  const targetPrice = Number($('targetPrice')?.value || prediction?.targetPrice);
  const usingCoinbaseTarget = isCoinbase && fresh && Number(prediction.targetPrice) === targetPrice;
  if (usingCoinbaseTarget) {
    const closeNote = prediction.closeTimeSource === 'ticker' ? 'ticker time' : 'estimated time';
    return { official: true, source: 'coinbase', label: `Coinbase Official · ${closeNote}`, className: 'pill good', reason: 'Fresh Coinbase target is active.' };
  }
  const activeTicker = String($('marketTicker')?.value || state.market?.ticker || '').toUpperCase();
  if (activeTicker.startsWith('LOCAL-BTC15M') || state.market?.source === 'local_coinbase_15m_window' || state.localPrediction?.targetPrice === targetPrice) {
    return { official: false, source: 'local', label: 'Local Fallback — tracker lock disabled', className: 'pill warn', reason: 'Waiting for fresh Coinbase target.' };
  }
  if (!fresh && isCoinbase) {
    return { official: false, source: 'stale_coinbase', label: 'Coinbase target stale — tracker lock disabled', className: 'pill warn', reason: 'Coinbase target exists but is stale.' };
  }
  if (Number.isFinite(targetPrice) && targetPrice > 0) {
    return { official: false, source: 'manual', label: 'Manual Target — tracker lock disabled', className: 'pill warn', reason: 'Manual targets are preview-only until Coinbase target matches.' };
  }
  return { official: false, source: 'missing', label: 'No target authority', className: 'pill bad', reason: 'No Coinbase target available.' };
}
...
function getNoLockBlockers(decision, candidate, remainingSec) {
  ...
  if (!authority.official) blockers.push(`Target source: ${authority.label}`);
...
function getLockReadinessSnapshot(decision = state.decision) {
  ...
  const adjustedPercent = authority.official ? percent : Math.min(percent, 35);
  return {
...
function updateOddsCache(sample) {
  ...
  state.oddsCache = {
    ...
    source: sample.source || state.coinbasePrediction?.source || 'coinbase_predictions',
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  if (!targetAuthority.official) {
    return {
      allow: false,
      reason: `Target source: ${targetAuthority.label}.`,
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
  ...
  const genesisClean = genesisGate.allow && genesisGate.pick === decision.action && genesisGate.settlementRisk !== 'High';
  const genesisElite = genesisClean && ['A+', 'A', 'B+'].includes(genesisGate.tradeGrade) && genesisGate.successChance >= 72;
...
function getGenesisGateSnapshot(decision = state.decision, remainingSec = getRemainingSec()) {
  ...
  const totalRisk = clampClient((100 - confidence) * 0.55 + lateWindowRisk * 0.6 + nearTargetRisk + whipsawRisk * 0.45 + wrongSideRisk, 0, 100);
...
function getOddsSnapshotForSide(side) {
  ...
  const directCandidates = [
    { value: state.coinbasePrediction?.[field], source: state.coinbasePrediction?.source || 'coinbase_predictions', ticker: state.coinbasePrediction?.ticker || '', status: 'Live scrape' },
  ...
  const candidates = [
    {
      ...
      source: state.coinbasePrediction?.source || 'coinbase_predictions',
...
function getTradingStyleRecommendation(decision = state.decision, candidate = null) {
  ...
  if (!authority.official) {
    return {
      ...
      status: 'Preview only',
      level: 'warn',
      message: `${style.label}: waiting for Coinbase Official target before money guidance.`,
      note: style.note
    };
  }
  if (!hasTradeSignal) {
  ...
  if (!freshOdds) {
    const base = officialLock ? `${style.label}: tracker locked ${officialLock.action}, but money value needs fresh matching odds.` : `${style.label}: signal present, but money value needs fresh matching odds.`;
  ...
  if (styleKey === 'safe_cents') {
    ...
    return {
      ...
      level: ok ? 'good' : 'quiet',
      message: ok ? `Safe / Cents: acceptable. ${payoutText}.` : `Safe / Cents: wait for official tracker lock or ${style.minReadiness}% readiness.`,
      note: style.note
    };
  }
  if (styleKey === 'value_hunter') {
    ...
    return {
      ...
      level: ok ? 'good' : 'warn',
      message: ok ? `Value Hunter: payout passes. ${payoutText}.` : `Value Hunter: tracker may be right, but payout is too small. Needs about +${formatDollar(style.minProfit)} or better.`,
      note: style.note
    };
  }
  if (styleKey === 'aggressive_value') {
    ...
    return {
      ...
      level: ok ? 'warn' : 'quiet',
      message: ok ? `Aggressive Value: possible early money idea, not official tracker lock. ${payoutText}.` : `Aggressive Value: needs ${style.minReadiness}% readiness, Genesis agreement, and about +${formatDollar(style.minProfit)} payout.`,
      note: style.note
    };
  }
  if (styleKey === 'scout') {
    ...
    return {
      ...
      level: ok ? 'warn' : 'quiet',
      message: ok ? `Scout: early high-risk idea only, not official tracker lock. ${payoutText}.` : `Scout: waiting for ${style.minReadiness}% readiness and any worthwhile payout.`,
      note: style.note
    };
  }
  ...
  return {
    ...
    message: balancedOk ? `Balanced: money entry acceptable. ${payoutText}.` : `Balanced: official lock may still count, but payout/value is thin.`,
    note: style.note
  };
}
...
function getCashOutAlert() {
  ...
  const rawOpposite = state.rawDecision?.action === opposite && Number(state.rawDecision?.confidence || 0) >= 76;
  const activeOpposite = state.decision?.action === opposite && Number(state.decision?.confidence || 0) >= 76;
  const flipRisk = Math.max(Number(state.rawDecision?.flipRisk || 0), Number(state.decision?.flipRisk || 0));
  const confidenceDrop = Number(lock.confidence || 0) - Number(state.decision?.confidence || 0) >= 18;
  const lockPrice = Number(lock.price);
  const movedAgainst = Number.isFinite(lockPrice)
    ? (side === 'OVER' ? latestPrice <= lockPrice - 22 : latestPrice >= lockPrice + 22)
    : false;
  let score = 0;
  const reasons = [];
  if (priceThroughTarget) { score += 3; reasons.push('price crossed target against lock'); }
  if (nearLine) { score += 1; reasons.push('lock cushion nearly gone'); }
  ...
  else if (flipRisk >= 60) { score += 1; reasons.push('flip risk rising'); }
  if (confidenceDrop) { score += 1; reasons.push('confidence dropped'); }
  if (movedAgainst) { score += 1; reasons.push('price moved against entry'); }
  if (score >= MIN_CASH_OUT_SCORE && (priceThroughTarget || recentOpposite >= 3 || rawOpposite || activeOpposite)) {
