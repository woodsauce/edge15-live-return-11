...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-18.zip';
...
function getTargetAuthorityStatus(now = Date.now()) {
  ...
  if (activeTicker.startsWith('LOCAL-BTC15M') || state.market?.source === 'local_coinbase_15m_window' || state.localPrediction?.targetPrice === targetPrice) {
    return { official: false, source: 'local', label: 'Local / Unverified — verify Coinbase target', className: 'pill warn', reason: 'Coinbase target is not confirmed. V18 warning only; tracker logic is not blocked.' };
  }
  if (!fresh && isCoinbase) {
    return { official: false, source: 'stale_coinbase', label: 'Coinbase scrape stale — verify target', className: 'pill warn', reason: 'Coinbase target exists but is stale. V18 warning only; tracker logic is not blocked.' };
  }
  if (Number.isFinite(targetPrice) && targetPrice > 0) {
    return { official: false, source: 'manual', label: 'Manual / Unverified — verify Coinbase target', className: 'pill warn', reason: 'Manual target in use. V18 warning only; tracker logic is not blocked.' };
  }
  return { official: false, source: 'missing', label: 'Target unverified — compare with Coinbase', className: 'pill warn', reason: 'No Coinbase target available. V18 warning only; tracker logic is not blocked.' };
}
...
function getNoLockBlockers(decision, candidate, remainingSec) {
  ...
  if (!authority.official) notes.push(`Target warning: ${authority.label}`);
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  // V18: target authority is a warning only. Do not block tracker locks/readiness/money guidance.
...
function getTradingStyleRecommendation(decision = state.decision, candidate = null) {
  ...
  const targetWarning = authority.official ? '' : ` Target warning: ${authority.label}.`;
  if (!hasTradeSignal) {
    return {
      styleKey,
