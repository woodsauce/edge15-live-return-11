...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-19.zip';
...
function hydrateSettings() {
  ...
  ['useCurrentAsTarget', 'start15m'].forEach((id) => {
    const btn = $(id);
    if (btn) { btn.disabled = true; btn.title = 'Manual target/time disabled in v19. Official KXBTC15M data required.'; }
  });
}
...
function updateLocalPredictionFromTicks(now = Date.now()) {
  ...
  // v19: local/generated targets are preview metadata only. They are never applied
...
function getTargetAuthorityStatus(now = Date.now()) {
  ...
  return { official: false, source: 'missing_official', label: 'Waiting for official KXBTC15M target', className: 'pill bad', reason: 'No verified official target available. v19 blocks official locks and scoring until data is verified.' };
}
...
async function loadCoinbasePrediction(silent = false) {
  ...
  try {
    ...
    const debug = $('apiDebug');
    if (debug) {
      debug.textContent = `Official target fetch failed: ${error.message}. v19 will not create a local target, tracker lock, money guidance, cash-out alert, or win/loss score until KXBTC15M data is verified.`;
    }
  }
}
...
function setTargetFromCurrent() {
  $('readiness').textContent = 'Manual target disabled in v19. Waiting for verified KXBTC15M target.';
}

function startManualCountdown(minutes = 15) {
  $('readiness').textContent = 'Manual countdown disabled in v19. Waiting for verified official market close time.';
}
