...
const APP_VERSION_FILE = 'edge15-genesis-innovation-lab-31-active-clone.zip';
const LEARNING_SAMPLE_TARGET = 25;
const STORE_KEY = 'edge15.clone.innovation31.records.v1';
const SETTINGS_KEY = 'edge15.clone.innovation31.settings.v1';
const LADDERS_KEY = 'edge15.clone.innovation31.ladders.v1';
const CURRENT_LADDER_KEY = 'edge15.clone.innovation31.currentLadder.v1';
const ODDS_CACHE_KEY = 'edge15.clone.innovation31.oddsCache.v1';
const MAX_TICKS = 900;
...
function getSixMinuteReadGuard(decision, candidate = {}) {
  ...
  // V31: this is intentionally a 6-minute read lock, not a loose early scout.
  ...
  if (grade === 'B' && String(genesis.settlementRisk || '') === 'High') blockers.push('6m B-grade read blocks only High settlement risk in Innovation Lab.');
  ...
  return { pass: blockers.length === 0, blockers, warnings, chartScore, successChance, stability, flipRisk, entryQuality, cushionScore, liveBalancedV31: true };
}
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  if (earlyGenesisLock) {
    return {
      allow: false,
      reason: '8m Genesis early lock is preview-only in V31; official money entry starts at the 6m read / 4m-2m second-chance windows.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
  ...
  // V31: if the strict second-chance path does not fire, reuse the balanced live-read guard late.
  ...
  if (remainingSec < SECOND_CHANCE_EARLIEST_SEC && remainingSec >= SECOND_CHANCE_LATEST_SEC) {
    ...
    if (lateCandidate.allow) return { ...lateCandidate, source: '4m-2m live balanced catch lock', reason: '4m-2m live balanced catch lock: late signal cleaned up under V31 guardrails.' };
  }
  if (tenMinutePreviewOnly) {
    return {
      allow: false,
      reason: '10m Genesis elite is preview-only; V31 waits for the 6m read or late second-chance lock.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
