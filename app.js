...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-15.zip';
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const sixMinuteMoneyLock = remainingSec <= 420 && remainingSec >= GENESIS_MONEY_LATEST_SEC && baseClean && entryQuality >= 52 && rightSideOrEliteReclaim && cushion.score >= 55 && genesisClean;
  const tenMinuteElite = minute === 10 && confidence >= 92 && stability >= 90 && flipRisk <= 48 && entryQuality >= 76 && heldMs >= 30000 && rightSide && cushion.score >= 90 && genesisElite;
  const earlyGenesisLock = remainingSec <= 540 && remainingSec > 420 && confidence >= 62 && flipRisk <= 52 && entryQuality >= 70 && heldMs >= 30000 && rightSide && cushion.score >= 70 && genesisElite;
  const emergency = remainingSec >= GENESIS_MONEY_LATEST_SEC && confidence >= 90 && flipRisk <= 28 && entryQuality >= 84 && rightSide && cushion.score >= 105 && genesisElite;
