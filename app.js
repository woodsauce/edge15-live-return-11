...
const APP_VERSION_FILE = 'edge15-genesis-balanced-29-six-minute-read.zip';
...
const ELITE_MIN_STABILITY = 78;
const ELITE_MIN_ENTRY_QUALITY = 78;
const ELITE_MAX_FLIP_RISK = { OVER: 42, UNDER: 42 };
const ELITE_MIN_CHART_SCORE = { OVER: 72, UNDER: 72 };
const ELITE_MIN_SUCCESS_CHANCE = { OVER: 72, UNDER: 72 };
...
function getEliteAccuracyGuard(decision, candidate = {}, source = '') {
  ...
  if (grade === 'B') {
    ...
    if (chartScore < 74) blockers.push(`B-grade requires stronger chart support: ${chartScore.toFixed(1)} below 74.`);
  }
...
function getSixMinuteReadGuard(decision, candidate = {}) {
  ...
  // V29: this is intentionally a 6-minute read lock, not a loose early scout.
  ...
  if (confidence < 94) blockers.push(`6m read confidence ${confidence.toFixed(1)} below 94.`);
  if (stability < 54) blockers.push(`6m read stability ${stability.toFixed(1)} below 54.`);
  if (flipRisk > 58) blockers.push(`6m read flip risk ${flipRisk.toFixed(1)} above 58.`);
  if (entryQuality < 70) blockers.push(`6m read entry quality ${entryQuality.toFixed(1)} below 70.`);
  if (!chart.available || !chart.supports || chart.conflict) blockers.push('6m read requires chart support without conflict.');
  if (chartScore < 61) blockers.push(`6m read chart score ${chartScore.toFixed(1)} below 61.`);
  if (cushionScore < 280) blockers.push(`6m read requires huge cushion; score ${cushionScore.toFixed(0)} below 280.`);
  ...
  if (successChance < 70) blockers.push(`6m read Genesis success ${successChance.toFixed(1)} below 70.`);
  if (side === 'OVER' && Number.isFinite(entryScore) && entryScore < 76) blockers.push(`6m OVER entryScore ${entryScore.toFixed(0)} below 76.`);
  if (side === 'UNDER' && Number.isFinite(entryScore) && entryScore > 25) blockers.push(`6m UNDER entryScore ${entryScore.toFixed(0)} above 25.`);
  if (vwapSide && vwapSide !== side) blockers.push(`6m read VWAP is against ${side}.`);
  if (targetCrosses > 16) blockers.push(`6m read target crosses ${targetCrosses} above 16.`);
  else if (targetCrosses > 6) warnings.push(`Target crossed ${targetCrosses} times; accepted only because cushion is huge.`);
  if (cushionVelocity < -6) blockers.push(`6m cushion velocity ${cushionVelocity.toFixed(1)} is deteriorating too fast.`);
  if (value.available === false) blockers.push(`Fresh matching odds required: ${value.reason || value.status || 'unavailable'}.`);
  if (Number.isFinite(oddsAgeSec) && oddsAgeSec > ELITE_MAX_ODDS_AGE_SEC) blockers.push(`Odds age ${oddsAgeSec}s above ${ELITE_MAX_ODDS_AGE_SEC}s.`);
  ...
  return { pass: blockers.length === 0, blockers, warnings, chartScore, successChance, stability, flipRisk, entryQuality, cushionScore };
}
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  if (earlyGenesisLock) {
    return {
      allow: false,
      reason: '8m Genesis early lock is preview-only in V29; official money entry starts at the 6m read / 4m-2m second-chance windows.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
  ...
  if (tenMinutePreviewOnly) {
    return {
      allow: false,
      reason: '10m Genesis elite is preview-only; V29 waits for the 6m read or late second-chance lock.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
