...
const APP_VERSION_FILE = 'edge15-genesis-master-machine-38-full-launch.zip';
...
function getMasterSensorStack(decision, candidate = {}, remainingSec = getRemainingSec()) {
  ...
  const oddsConfirm = oddsFresh && oddsPercent > 0 && oddsPercent < (side === 'OVER' ? 88 : 92);
  if (oddsConfirm) { score += 4; addSensor('Odds Pressure', 'confirm', `${oddsPercent.toFixed(0)}¢ fresh display-only odds`, 4); }
  else if (value.available === false) addSensor('Odds Pressure', 'warn', 'odds unavailable; display-only', 0);
  else addSensor('Odds Pressure', 'warn', 'odds are thin/expensive; display-only', 0);
