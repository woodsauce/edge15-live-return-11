...
const APP_VERSION_FILE = 'edge15-money-tools-14.zip';
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const valueGate = getMoneyValueGate(decision);
  ...
  const cushion = Math.abs(latestPrice - targetPrice);
  ...
  const tenMinuteElite = minute === 10 && confidence >= 92 && stability >= 92 && flipRisk <= 42 && entryQuality >= 78 && heldMs >= 30000 && rightSide && cushion >= eliteCushion;
  ...
  const approve = (source) => {
    if (!valueGate.allow) return { allow: false, reason: valueGate.reason, minute, entryQuality, heldMs, valueSnapshot: valueGate.snapshot };
    return { allow: true, source, minute, entryQuality, heldMs, valueSnapshot: valueGate.snapshot };
  };

  if (tenMinuteElite) return approve('10m elite lock');
  if (priority6m) return approve('6m priority lock');
  if (earlyStrong) return approve('early strong lock');
  if (lateStrong) return approve('late strong lock');
  if (emergency) return approve('emergency lock');
  return { allow: false, reason: 'Waiting for official lock', minute, entryQuality, heldMs, valueSnapshot: valueGate.snapshot };
}
...
function getExpectedValueSnapshot(decision = state.decision) {
  ...
  const odds = getOddsSnapshotForSide(decision.action);
  const winProbability = estimateWinProbability(decision);
  if (!odds.available || !Number.isFinite(winProbability)) {
    return { available: false, status: 'Odds missing', reason: 'No Coinbase odds available for EV check', side: decision.action };
  }
...
function getTenMinuteEliteStatus(decision = state.decision) {
  ...
  const eliteCushion = Math.max(18, targetPrice * 0.00028);
  const passes = Number(decision.confidence || 0) >= 92 && Number(decision.stability || 0) >= 92 && Number(decision.flipRisk || 100) <= 42 && Number(timing.entryQuality || 0) >= 78 && getHeldDurationMs(decision) >= 30000 && rightSide && cushion >= eliteCushion;
  return passes ? 'Ready' : 'Not elite enough';
}
...
function renderMoneyTools() {
  ...
  const odds = snapshot.available ? `${snapshot.oddsPercent}% ${snapshot.oddsStatus ? `· ${snapshot.oddsStatus}` : ''}` : 'Unavailable';
  const rows = [
    ['EV status', snapshot.available ? snapshot.status : snapshot.status || 'Waiting'],
    ['Odds cost', odds],
    ...
    ['Expensive rule', snapshot.available ? (valueGate.allow ? 'Pass' : 'Blocks lock') : 'No odds'],
...
function captureLiveReturnAtLock(action) {
  ...
  if (!Number.isFinite(oddsPercent) || oddsPercent <= 0 || oddsPercent >= 100) {
    return {
      ...
      side: normalized,
      capturedAt: Date.now(),
      source: state.coinbasePrediction?.source || state.orderbook?.source || state.oddsCache?.source || 'unavailable',
      oddsStatus: getOddsCacheStatus().label
    };
  }
  ...
  return {
    ...
    capturedAt: Date.now(),
    source: state.coinbasePrediction?.source || state.orderbook?.source || state.oddsCache?.source || 'coinbase_predictions',
    ticker: state.coinbasePrediction?.ticker || state.market?.ticker || state.oddsCache?.ticker || '',
    oddsAgeSec: cacheStatus.ageSec,
    oddsStatus: cacheStatus.label
  };
}
