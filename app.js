...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-16.zip';
...
function getLockReadinessSnapshot(decision = state.decision) {
  ...
  const needs = [];
  if (!rightSide) needs.push(`${decision.action} is wrong side of target`);
  if (!genesis.allow) needs.push(`Genesis gate: ${genesis.status}`);
  if (Number(cushion.score || 0) < LOCK_CUSHION_SCORE) needs.push(`Needs cushion: ${cushion.label}`);
  if (Number(decision.flipRisk || 100) > 58) needs.push('Flip risk too high');
  if (getHeldDurationMs(decision) < 30000) needs.push('Needs signal to hold longer');
  return {
    percent,
    label: `${percent}% ready`,
    reason: needs.slice(0, 3).join(' · ') || candidate.reason || 'Waiting for cleaner confirmation.',
    locked: false
  };
}
