...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-21.zip';
...
const UNDER_OPPORTUNITY_EASING = 0.07;
...
function hydrateSettings() {
  ...
  ['useCurrentAsTarget', 'start15m'].forEach((id) => {
    const btn = $(id);
    if (btn) { btn.disabled = true; btn.title = 'Manual target/time disabled in v21. Official KXBTC15M data required.'; }
  });
}
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const underOpportunityClean = decision.action !== 'UNDER' || (
    ...
    Number(chartSupport.underScore || 0) >= 60 &&
    Number(chartSupport.underCushionVelocity || 0) >= -4 &&
    Number(chartSupport.targetCrosses || 0) <= 4 &&
    Number(genesisGate.totalRisk || 100) <= 70
  );
...
function getGenesisGateSnapshot(decision = state.decision, remainingSec = getRemainingSec()) {
  ...
  const underOpportunity = pick === 'UNDER' && chart.available && chart.supports && Number(chart.underScore || 0) >= 60 && Number(chart.underCushionVelocity || 0) >= -4 && Number(chart.targetCrosses || 0) <= 4;
...
async function loadCoinbasePrediction(silent = false) {
  ...
  try {
    ...
    const debug = $('apiDebug');
    if (debug) {
      debug.textContent = `Official target fetch failed: ${error.message}. v21 will not create a local target, tracker lock, money guidance, cash-out alert, or win/loss score until KXBTC15M data is verified.`;
    }
  }
}
...
function getCashOutAlert() {
  ...
  if (cushionCollapsing) { score += isEmergencyLock ? 2 : 1; watchScore += 1; reasons.push('cushion collapsing'); }
  ...
  if (remainingSec <= 300 && remainingSec > 60 && watchScore >= 3 && (chartAgainst || cushionShrinking || nearLine || movedAgainst)) {
