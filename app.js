...
const APP_VERSION_FILE = 'edge15-genesis-elite-27-genesis020-calibrated.zip';
...
const ELITE_MIN_STABILITY = 80;
const ELITE_MIN_ENTRY_QUALITY = 82;
const ELITE_MAX_FLIP_RISK = { OVER: 36, UNDER: 34 };
const ELITE_MIN_CHART_SCORE = { OVER: 76, UNDER: 76 };
const ELITE_MIN_SUCCESS_CHANCE = { OVER: 80, UNDER: 80 };
const ELITE_STRONG_CUSHION_SCORE = 115;
const ELITE_MAX_ODDS_AGE_SEC = 20;
const DISABLED_MONEY_LOCKS = ['10m genesis elite lock', '8m genesis early lock'];
const NO_ENTRY_LOCKS = ['late proof lock · no-entry'];
const GENESIS020_OVER_MIN_SCORE = 80;
const GENESIS020_UNDER_MAX_SCORE = 25;
const GENESIS020_ELITE_OVER_SCORE = 95;
const GENESIS020_ELITE_UNDER_SCORE = 15;
const GENESIS020_STANDARD_GRADES = ['A+', 'A', 'B+'];
const GENESIS020_WATCH_ONLY_GRADES = ['B', 'C', 'D', 'F'];
const SCOUT_MIN_STABILITY = 80;
const SCOUT_MIN_CHART_BY_SIDE = { OVER: 76, UNDER: 76 };
const SCOUT_MAX_FLIP_BY_SIDE = { OVER: 34, UNDER: 32 };
...
function getGenesis020Calibration(decision, genesis = {}, source = '') {
  ...
  if (gradeWatchOnly) blockers.push(`Genesis-020 calibration makes ${grade} grade watch-only; B-grade ran below elite accuracy.`);
  else if (!gradePass) blockers.push(`Genesis-020 grade ${grade || 'unknown'} is not approved for official entry.`);
  if (successChance < ELITE_MIN_SUCCESS_CHANCE[side]) blockers.push(`Genesis success ${successChance.toFixed(1)} below calibrated ${ELITE_MIN_SUCCESS_CHANCE[side]} for ${side}.`);
  if (standardScorePass && !eliteScorePass) warnings.push('Genesis-020 score passes standard calibration, but it is not the 95+/15 ultra tier.');
...
function getEliteAccuracyGuard(decision, candidate = {}, source = '') {
  ...
  if (['C', 'D', 'F'].includes(grade)) blockers.push(`Trade grade ${grade} is not elite-ready.`);
  ...
  if (successChance < ELITE_MIN_SUCCESS_CHANCE[side]) blockers.push(`Genesis success ${successChance.toFixed(1)} below ${ELITE_MIN_SUCCESS_CHANCE[side]} for ${side}.`);
  ...
  if (value.available === false) blockers.push(`Fresh matching odds required for data quality: ${value.reason || value.status || 'unavailable'}.`);
  if (Number.isFinite(oddsAgeSec) && oddsAgeSec > ELITE_MAX_ODDS_AGE_SEC) blockers.push(`Odds age ${oddsAgeSec}s above ${ELITE_MAX_ODDS_AGE_SEC}s.`);
  ...
  if (nonEmergency && targetCrosses > 2) blockers.push(`Too many target crosses (${targetCrosses}) for elite lock.`);
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const genesisClean = genesisGate.allow && genesisGate.pick === side && genesisGate.settlementRisk !== 'High' && Number(genesisGate.successChance || 0) >= ELITE_MIN_SUCCESS_CHANCE[side];
  ...
  const commonStrict = baseClean && entryQuality >= ELITE_MIN_ENTRY_QUALITY && rightSideOnly && strongCushion && chartClean && genesisClean && gradeClean && genesis020.pass && cushionVelocity >= -2;
  ...
  const approve = (source, entryEligible = remainingSec >= GENESIS_MONEY_LATEST_SEC) => {
    ...
    if (!eliteGuard.pass) {
      return {
        ...
        allow: false,
        eliteGuard,
        reason: `Elite guard blocked: ${eliteGuard.blockers.slice(0, 3).join(' · ')}`
      };
    }
    return { ...candidate, eliteGuard, reason: `${source}: elite accuracy guardrails passed.` };
  };
  ...
  if (earlyGenesisLock) {
    return {
      allow: false,
      reason: '8m Genesis early lock is preview-only in V27; Genesis-020 timing data showed early raw checkpoints were not 90%+.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
  ...
  if (tenMinutePreviewOnly) {
    return {
      allow: false,
      reason: '10m Genesis elite is preview-only until it proves 90%+ again.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
  ...
  const sideNote = rightSide ? 'Waiting for elite official lock' : `${decision.action} is on wrong side of target`;
  const guardNote = eliteGuard.blockers.length ? ` Elite guard: ${eliteGuard.blockers.slice(0, 3).join(' · ')}.` : '';
  return {
    allow: false,
