...
const APP_VERSION_FILE = 'edge15-genesis-model-tournament-36-clean-tracking-fix.zip';
const LEARNING_SAMPLE_TARGET = 25;
const STORE_KEY = 'edge15.clone.tournament36.records.v1';
const SETTINGS_KEY = 'edge15.clone.tournament36.settings.v1';
const LADDERS_KEY = 'edge15.clone.tournament36.ladders.v1';
const CURRENT_LADDER_KEY = 'edge15.clone.tournament36.currentLadder.v1';
const ODDS_CACHE_KEY = 'edge15.clone.tournament36.oddsCache.v1';
const MAX_TICKS = 900;
...
const TOURNAMENT_RECORDS_KEY = 'edge15.clone.tournament36.modelRecords.v1';
const TRACKER_LOCK_VERSION = 36;
...
function maybeLockTournamentModels(decision, remainingSec) {
  ...
  for (const item of decisions) {
    ...
    ladder.modelLocks[item.modelId] = {
      ...
      lockedAt: Date.now(),
      trackerLockVersion: TRACKER_LOCK_VERSION,
      lockType: item.fallbackLock ? 'live_fallback' : 'live_primary',
      recoveredFromEntryMinute: false,
...
function getTournamentScoringLockInfo(ladder) {
  const liveLocks = ladder?.modelLocks || {};
  const recoveredAll = recoverTournamentLocksFromEntryMinutes(ladder);
  const recoveredLocks = {};
  for (const [modelId, lock] of Object.entries(recoveredAll || {})) {
    if (liveLocks[modelId]) continue;
    recoveredLocks[modelId] = {
      ...lock,
      lockType: 'recovered',
      recoveredFromEntryMinute: true
    };
  ...
  if (ladder) {
    ladder.recoveredModelLocks = recoveredLocks;
  }
  return {
    liveLocks,
    recoveredLocks,
    scoringLocks: { ...recoveredLocks, ...liveLocks },
    liveCount: Object.keys(liveLocks).length,
    recoveredCount: Object.keys(recoveredLocks).length
  };
}

function getTournamentScoringLocks(ladder) {
  return getTournamentScoringLockInfo(ladder).scoringLocks;
}

function scoreTournamentModelResults(ladder, finalSide) {
  const results = {};
  const lockInfo = getTournamentScoringLockInfo(ladder);
  const locks = lockInfo.scoringLocks;
  for (const model of TOURNAMENT_MODEL_DEFS) {
    const lock = locks[model.id] || null;
    const lockType = lockInfo.liveLocks[model.id] ? (lockInfo.liveLocks[model.id].lockType || 'live') : (lockInfo.recoveredLocks[model.id] ? 'recovered' : 'none');
    ...
    results[model.id] = {
      ...
      lockType,
      recoveredFromEntryMinute: lockType === 'recovered',
      isLiveLock: lockType === 'live' || lockType === 'live_primary' || lockType === 'live_fallback'
    };
  }
  return results;
}
...
function addTournamentRecordsFromLadder(ladder) {
  ...
  const scoringInfo = getTournamentScoringLockInfo(ladder);
  const scoringLocks = scoringInfo.scoringLocks;
  const newRecords = [];
  for (const model of TOURNAMENT_MODEL_DEFS) {
    ...
    newRecords.push({
      ...
      lockType: result.lockType || (lock?.recoveredFromEntryMinute ? 'recovered' : (lock ? 'live' : 'none')),
      recoveredFromEntryMinute: Boolean(result.recoveredFromEntryMinute || lock?.recoveredFromEntryMinute),
      isLiveLock: Boolean(result.isLiveLock || (lock && !lock.recoveredFromEntryMinute)),
...
function getTournamentSourceRecords() {
  return getOfficialTrackerRecords()
    .filter((record) => record?.recordType === 'auto_ladder')
    .filter((record) => record?.finalSide);
}

function buildTournamentRecordsFromActiveRecords() {
  const rows = [];
  const sourceRecords = getTournamentSourceRecords();
  for (const record of sourceRecords) {
    const finalSide = record.finalSide;
    const results = record.tournamentResults || scoreTournamentModelResults(record, finalSide);
    const lockInfo = getTournamentScoringLockInfo(record);
    const locks = lockInfo.scoringLocks || {};
    for (const model of TOURNAMENT_MODEL_DEFS) {
      const result = results[model.id] || {};
      const lock = locks[model.id] || null;
      rows.push({
        id: `${record.id || record.windowKey}-${model.id}`,
        windowVersionKey: `${record.windowKey}|${TRACKER_LOCK_VERSION}`,
        recordType: 'tournament_model',
        trackerLockVersion: TRACKER_LOCK_VERSION,
        modelId: model.id,
        label: model.label,
        ...
        flipRisk: lock?.flipRisk ?? null,
        entryQuality: lock?.entryQuality ?? null,
        settledAt: record.settledAt || record.ts || Date.now()
      });
    }
  }
  return rows;
...
function summarizeTournamentIntegrity() {
  const sourceRecords = getTournamentSourceRecords();
  const rows = buildTournamentRecordsFromActiveRecords();
  const expectedModelRows = sourceRecords.length * TOURNAMENT_MODEL_DEFS.length;
  return {
    source: 'activeVersionRecords.tournamentResults',
    activeWindows: sourceRecords.length,
    modelsPerWindow: TOURNAMENT_MODEL_DEFS.length,
    expectedModelRows,
    modelRows: rows.length,
    missingModelRows: Math.max(0, expectedModelRows - rows.length),
    liveLocks: rows.filter((row) => row.isLiveLock).length,
    recoveredLocks: rows.filter((row) => row.recoveredFromEntryMinute).length,
    trueSkips: rows.filter((row) => row.result === 'skipped').length
  };
}
...
function summarizeTournamentModels() {
  const summary = {};
  for (const model of TOURNAMENT_MODEL_DEFS) {
    summary[model.id] = { modelId: model.id, label: model.label, family: model.family, wins: 0, losses: 0, skipped: 0, void: 0, taken: 0, completed: 0, liveLocks: 0, recoveredLocks: 0, winRate: 0, tradeRate: 0, liveLockRate: 0 };
  }
  for (const record of buildTournamentRecordsFromActiveRecords()) {
    const row = summary[record.modelId];
    if (!row) continue;
    row.completed += 1;
    if (record.isLiveLock) row.liveLocks += 1;
    if (record.recoveredFromEntryMinute) row.recoveredLocks += 1;
  ...
  return Object.values(summary).map((row) => ({
    ...
    liveLockRate: row.completed ? round2((row.liveLocks / row.completed) * 100) : 0
...
function renderTournamentDashboard(decision = state.decision) {
  ...
  const integrityEl = $('tournamentIntegrity');
  if (!currentEl && !statsEl && !integrityEl) return;
  const ladder = state.currentLadder;
  ...
  if (integrityEl) {
    const integrity = summarizeTournamentIntegrity();
    integrityEl.textContent = `Tracking source: ${integrity.source} · windows ${integrity.activeWindows} · rows ${integrity.modelRows}/${integrity.expectedModelRows} · live locks ${integrity.liveLocks} · recovered ${integrity.recoveredLocks} · true skips ${integrity.trueSkips}`;
  }
  if (currentEl) {
  ...
  if (statsEl) {
    statsEl.innerHTML = TOURNAMENT_MODEL_DEFS.map((model) => {
      ...
      return `<div class="mini-row tournament-stat-row"><strong>${escapeHtml(model.label)}</strong><span>W ${row.wins} / L ${row.losses} / S ${row.skipped} · <b class="${rateClass}">${row.winRate}%</b> · trade ${row.tradeRate}% (${row.taken}/${row.completed}) · live ${row.liveLocks || 0} / rec ${row.recoveredLocks || 0}</span></div>`;
    }).join('');
  }
}
...
function finalizeCurrentLadder(reason = 'closed') {
  ...
  const completed = {
    ...
    recoveredModelLocks: ladder.recoveredModelLocks || {},
    settledAt: Date.now(),
    settleReason: reason
  };
...
function renderAppQrCode() {
  ...
  const copyBtn = $('copyAppUrl');
  if (!img && !urlEl && !copyBtn) return;
  const url = window.location.href;
  if (img) {
    img.src = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&margin=8&data=${encodeURIComponent(url)}`;
    img.alt = 'QR code to open this Edge15 app on a phone';
  }
