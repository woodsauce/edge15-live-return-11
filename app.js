...
const APP_VERSION_FILE = 'edge15-genesis-regime-lab-32-microstructure-clone.zip';
const LEARNING_SAMPLE_TARGET = 25;
const STORE_KEY = 'edge15.clone.regimelab32.records.v1';
const SETTINGS_KEY = 'edge15.clone.regimelab32.settings.v1';
const LADDERS_KEY = 'edge15.clone.regimelab32.ladders.v1';
const CURRENT_LADDER_KEY = 'edge15.clone.regimelab32.currentLadder.v1';
const ODDS_CACHE_KEY = 'edge15.clone.regimelab32.oddsCache.v1';
const MAX_TICKS = 900;
...
function getSixMinuteReadGuard(decision, candidate = {}) {
  ...
  // V32: this is intentionally a 6-minute read lock, not a loose early scout.
  ...
  if (grade === 'B' && String(genesis.settlementRisk || '') === 'High') blockers.push('6m B-grade read blocks only High settlement risk in Regime Lab.');
  ...
  return { pass: blockers.length === 0, blockers, warnings, chartScore, successChance, stability, flipRisk, entryQuality, cushionScore, liveBalancedV32: true };
}
...
function getRegimeSnapshot(side, chart = {}, cushion = {}, genesis = {}) {
  const s = signForSide(side);
  ...
  else if (trendStructure || (alignedMomentum && cushionScore >= 100 && tapeSupports)) regime = 'directional-lean';
  const regimeScore = clampClient(
    chartScore * 0.30 +
    cushionScore * 0.12 +
    Number(genesis.successChance || 50) * 0.18 +
    (tape.available ? tape.score : 55) * 0.22 +
  ...
  return {
    ...
    label: `${regime} · score ${round2(regimeScore)} · ${tape.label}`
  };
}

function getRegimeLabGuard(decision, candidate = {}) {
  const side = String(decision?.action || '').toUpperCase();
  const blockers = [];
  const warnings = [];
  if (!['OVER', 'UNDER'].includes(side)) return { pass: false, blockers: ['No active OVER/UNDER side.'], warnings };
  ...
  const entryQuality = Number(candidate.entryQuality || getTimingSignal(decision).entryQuality || 0);
  const successChance = Number(genesis.successChance || 0);
  const regime = getRegimeSnapshot(side, chart, cushion, genesis);
  const entryScore = Number(genesis.entryScore);

  if (!targetAuthority.official) blockers.push(targetAuthority.reason || 'Official target authority required.');
  if (!rightSide) blockers.push(`${side} must already be on the correct side of target.`);
  if (!genesis.allow || genesis.pick !== side) blockers.push('Regime Lab requires Genesis agreement.');
  if (!GENESIS020_STANDARD_GRADES.includes(grade)) blockers.push(`Regime Lab grade ${grade || 'unknown'} is not allowed.`);
  if (grade === 'B' && String(genesis.settlementRisk || '') === 'High') blockers.push('Regime Lab B-grade blocks High settlement risk.');
  if (confidence < 68) blockers.push(`Regime confidence ${confidence.toFixed(1)} below 68.`);
  if (entryQuality < 48) blockers.push(`Regime entry quality ${entryQuality.toFixed(1)} below 48.`);
  if (flipRisk > 78) blockers.push(`Regime flip risk ${flipRisk.toFixed(1)} above 78.`);
  if (successChance < 52) blockers.push(`Regime Genesis success ${successChance.toFixed(1)} below 52.`);
  if (side === 'OVER' && Number.isFinite(entryScore) && entryScore < 60) blockers.push(`Regime OVER entryScore ${entryScore.toFixed(0)} below 60.`);
  if (side === 'UNDER' && Number.isFinite(entryScore) && entryScore > 42) blockers.push(`Regime UNDER entryScore ${entryScore.toFixed(0)} above 42.`);
  if (!['trend-continuation', 'squeeze-breakout', 'directional-lean'].includes(regime.regime)) blockers.push(`Regime ${regime.regime} is not tradeable.`);
  if (regime.regimeScore < 58) blockers.push(`Regime score ${regime.regimeScore.toFixed(1)} below 58.`);
  if (regime.exhaustion) blockers.push('Regime Lab blocks exhaustion/snapback state.');
  if (regime.reversalRisk && regime.regimeScore < 72) blockers.push('Regime Lab blocks reversal risk unless regime score is very strong.');
  if (regime.targetCrosses > 20) blockers.push(`Regime target crosses ${regime.targetCrosses} above 20.`);
  if (Number(cushion.score || 0) < 70) blockers.push('Regime Lab requires at least usable cushion.');
  ...
  if (stability < 55) warnings.push('Regime Lab accepted noisy stability only because regime/tape/cushion aligned.');
  ...
  return { pass: blockers.length === 0, blockers, warnings, regime, liveRegimeLabV32: true };
}
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const regimeLabWindow = remainingSec <= 450 && remainingSec >= 150;
  ...
  const approveRegimeLab = () => {
    const candidate = {
      allow: true,
      source: 'V32 regime + tape pressure lock',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
    ...
    const regimeGuard = getRegimeLabGuard(decision, candidate);
    if (!regimeGuard.pass) {
      return {
        ...candidate,
        allow: false,
        regimeGuard,
        reason: `Regime Lab blocked: ${regimeGuard.blockers.slice(0, 3).join(' · ')}`
      };
    }
    return {
      ...candidate,
      regimeGuard,
      eliteGuard: regimeGuard,
      reason: `V32 regime + tape pressure lock: ${regimeGuard.regime.label}`
    };
  };
  ...
  if (earlyGenesisLock) {
    return {
      allow: false,
      reason: '8m Genesis early lock is preview-only in V32; official money entry starts at the 6m read / 4m-2m second-chance windows.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
  ...
  if (regimeLabWindow) {
    const regimeCandidate = approveRegimeLab();
    if (regimeCandidate.allow) return regimeCandidate;
  }
  ...
  // V32: if the strict second-chance path does not fire, reuse the balanced live-read guard late.
  ...
  if (remainingSec < SECOND_CHANCE_EARLIEST_SEC && remainingSec >= SECOND_CHANCE_LATEST_SEC) {
    ...
    if (lateCandidate.allow) return { ...lateCandidate, source: '4m-2m live balanced catch lock', reason: '4m-2m live balanced catch lock: late signal cleaned up under V32 guardrails.' };
  }
  if (tenMinutePreviewOnly) {
    return {
      allow: false,
      reason: '10m Genesis elite is preview-only; V32 waits for the 6m read or late second-chance lock.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
...
function renderChartSignalLayer(decision = state.decision) {
  ...
  const regime = ['OVER', 'UNDER'].includes(decision?.action) ? getRegimeSnapshot(decision.action, getChartSignalSupport(decision), getTimeAdjustedCushionSnapshot(decision.action, Number(state.ticks.at(-1)?.price), Number($('targetPrice')?.value), getRemainingSec(), decision), getGenesisGateSnapshot(decision, getRemainingSec())) : null;
  const rows = [
    ['Micro trend', `${chart.direction} · OVER ${chart.overScore} / UNDER ${chart.underScore}`],
    ['Regime Lab', regime ? regime.label : 'Waiting for side'],
