...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-24.zip';
...
const SCOUT_MIN_CONFIDENCE = 78;
const SCOUT_MAX_FLIP_RISK = 66;
const SCOUT_MIN_ENTRY_QUALITY = 58;
const SCOUT_MIN_CHART_SCORE = 66;
const SCOUT_MAX_TARGET_CROSSES = 3;
const SCOUT_MIN_CUSHION_SCORE = 25;
const SCOUT_MIN_GENESIS_SUCCESS = 58;
const SCOUT_MAX_GENESIS_RISK = 78;
...
const OVER_REVERSAL_MIN_CHART_SCORE = 68;
const OVER_REVERSAL_MAX_TARGET_CROSSES = 2;
const OVER_REVERSAL_MIN_CUSHION_VELOCITY = -4;
const UNDER_BASE_MIN_CHART_SCORE = 62;
const UNDER_BASE_MAX_TARGET_CROSSES = 3;
const UNDER_BASE_MIN_CUSHION_VELOCITY = -4;
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const underBaseClean = decision.action !== 'UNDER' || (
    rightSide &&
    (!chartSupport.available || (
      Number(chartSupport.underScore || 0) >= UNDER_BASE_MIN_CHART_SCORE &&
      Number(chartSupport.underCushionVelocity || 0) >= UNDER_BASE_MIN_CUSHION_VELOCITY &&
      Number(chartSupport.targetCrosses || 0) <= UNDER_BASE_MAX_TARGET_CROSSES &&
      String(chartSupport.vwapSide || '') !== 'OVER' &&
      Number(chartSupport.momentum60s || 0) <= 8
    )) &&
    Number(genesisGate.totalRisk || 100) <= 72
  );
  const underOpportunityClean = decision.action === 'UNDER' && underBaseClean && chartSupport.available && chartSupport.supports &&
    Number(chartSupport.underScore || 0) >= 72 &&
    Number(chartSupport.underCushionVelocity || 0) >= 0 &&
    Number(chartSupport.targetCrosses || 0) <= 2 &&
    String(chartSupport.vwapSide || '') === 'UNDER' &&
    Number(chartSupport.momentum60s || 0) <= 2 &&
    Number(genesisGate.totalRisk || 100) <= 68;
  const overReversalClean = decision.action !== 'OVER' || (
    (!chartSupport.available || (
      Number(chartSupport.overScore || 0) >= OVER_REVERSAL_MIN_CHART_SCORE &&
      Number(chartSupport.overCushionVelocity || 0) >= OVER_REVERSAL_MIN_CUSHION_VELOCITY &&
      Number(chartSupport.targetCrosses || 0) <= OVER_REVERSAL_MAX_TARGET_CROSSES &&
      String(chartSupport.vwapSide || '') !== 'UNDER'
    )) &&
    Number(genesisGate.nearTargetRisk || 0) <= 18 &&
    (Number(cushion.score || 0) >= 82 || remainingSec <= 180)
  );
  const hugeLateCushion = remainingSec <= 180 && rightSide && cushion.score >= LATE_HUGE_CUSHION_SCORE && confidence >= 58 && flipRisk <= 72;
  const underCushionEase = decision.action === 'UNDER' && underOpportunityClean ? (1 - UNDER_OPPORTUNITY_EASING) : 1;
  const sixMinuteCushionRequired = 55 * underCushionEase;
  const earlyCushionRequired = 70 * underCushionEase;
  const eliteCushionRequired = 90 * underCushionEase;
  const sixMinuteMoneyLock = remainingSec <= 420 && remainingSec >= GENESIS_MONEY_LATEST_SEC && baseClean && entryQuality >= 52 && rightSideOrEliteReclaim && cushion.score >= sixMinuteCushionRequired && genesisClean && overSixClean && overReversalClean && underBaseClean;
  const tenMinuteElite = minute === 10 && confidence >= 92 && stability >= 90 && flipRisk <= 48 && entryQuality >= 76 && heldMs >= 30000 && rightSide && cushion.score >= eliteCushionRequired && genesisElite && overEarlyClean && overReversalClean && underBaseClean;
  const earlyGenesisLock = remainingSec <= 540 && remainingSec > 420 && confidence >= 62 && flipRisk <= 52 && entryQuality >= 70 && heldMs >= 30000 && rightSide && cushion.score >= earlyCushionRequired && genesisElite && overEarlyClean && overReversalClean && underBaseClean;
  const emergencyCushionRequired = 105 * EMERGENCY_CUSHION_TIGHTENING;
  const emergencyCushionVelocity = decision.action === 'UNDER' ? Number(chartSupport.underCushionVelocity || 0) : Number(chartSupport.overCushionVelocity || 0);
  const emergencyChartScore = decision.action === 'UNDER' ? Number(chartSupport.underScore || 0) : Number(chartSupport.overScore || 0);
  const emergencyClean = (!chartSupport.available || (emergencyChartScore >= 70 && emergencyCushionVelocity >= -2 && Number(chartSupport.targetCrosses || 0) <= 3));
  const emergency = remainingSec >= GENESIS_MONEY_LATEST_SEC && confidence >= 90 && flipRisk <= 28 && entryQuality >= 84 && rightSide && cushion.score >= emergencyCushionRequired && genesisElite && overEarlyClean && overReversalClean && underBaseClean && emergencyClean;
...
function getScoutCandidate(decision = state.decision, remainingSec = getRemainingSec()) {
  ...
  const genesisNotFighting = genesis.pick === action || (chart.available && chart.supports && Number(genesis.totalRisk || 100) <= SCOUT_MAX_GENESIS_RISK && Number(genesis.successChance || 0) >= SCOUT_MIN_GENESIS_SUCCESS);
  ...
  if (Number(decision.flipRisk || 100) > SCOUT_MAX_FLIP_RISK) blockers.push(`flip risk above ${SCOUT_MAX_FLIP_RISK}%`);
  if (Number(timing.entryQuality || 0) < SCOUT_MIN_ENTRY_QUALITY) blockers.push(`entry quality below ${SCOUT_MIN_ENTRY_QUALITY}%`);
  if (!rightSide && Number(cushion.score || 0) < SCOUT_MIN_CUSHION_SCORE) blockers.push('not cleanly on the right side of target');
  if (!chart.available || !chart.supports || supportScore < SCOUT_MIN_CHART_SCORE) blockers.push(`chart support below ${SCOUT_MIN_CHART_SCORE}`);
  if (Number(chart.targetCrosses || 0) > SCOUT_MAX_TARGET_CROSSES) blockers.push('too much opening target chop');
  if (cushionVelocity < -3) blockers.push('cushion already shrinking');
  if (!genesisNotFighting) blockers.push('Genesis is fighting the scout direction');
  ...
  return {
    ...
    status: blockers.length === 0 ? 'Scout ready' : 'Watching',
    reason: blockers.length ? blockers.slice(0, 3).join(' · ') : `${action} scout setup passed early-risk checks.`,
    action,
...
function maybeLockScout(decision = state.decision, remainingSec = getRemainingSec()) {
  ...
  ladder.scoutLock = {
    ...
    source: 'first 3-minute scout lock',
    risk: 'High',
...
function getTradingStyleRecommendation(decision = state.decision, candidate = null) {
  ...
  if (styleKey === 'value_hunter') {
    ...
    return {
      styleKey,
      styleLabel: style.label,
      status: ok ? 'Lock valid · value entry' : (officialLock || candidateSnapshot.allow) ? 'Lock valid · low payout warning' : 'Wait',
      level: ok ? 'good' : (officialLock || candidateSnapshot.allow) ? 'warn' : 'quiet',
      message: (ok ? `${validLockText}Value Hunter note only: payout passes. ${payoutText}.` : (officialLock || candidateSnapshot.allow) ? `${validLockText}Value Hunter note only: low payout warning. Needs about +${formatDollar(style.minProfit)} or better. ${payoutText}.` : `Value Hunter note only: waiting for valid tracker setup.`) + targetWarning,
      note: style.note
    };
  }
  if (styleKey === 'aggressive_value') {
    ...
    return {
      ...
      level: ok ? 'warn' : 'quiet',
      message: (ok ? `${validLockText}Aggressive Value note only: possible higher-risk money idea. ${payoutText}.` : `Aggressive Value note only: needs ${style.minReadiness}% readiness, Genesis agreement, and about +${formatDollar(style.minProfit)} payout.`) + targetWarning,
      note: style.note
    };
  }
  if (styleKey === 'scout') {
  ...
  const isBalancedCandidate = officialLock || candidateSnapshot.allow;
  const thinBalancedPayout = snapshot.available && isBalancedCandidate && profit > 0 && profit < style.minProfit;
  const expensiveWarning = snapshot.available && isBalancedCandidate && snapshot.tooExpensive;
  const balancedOk = isBalancedCandidate && profit >= style.minProfit;
  return {
    styleKey,
    styleLabel: style.label,
    status: balancedOk ? 'Lock valid · money OK' : (thinBalancedPayout || expensiveWarning) ? 'Lock valid · low payout warning' : 'Wait',
    level: balancedOk ? 'good' : (thinBalancedPayout || expensiveWarning) ? 'warn' : 'quiet',
    message: (balancedOk
      ? `${validLockText}Balanced note only: money OK. ${payoutText}.`
      : (thinBalancedPayout || expensiveWarning)
        ? `${validLockText}Balanced note only: low payout / contract expensive warning. ${payoutText}.`
...
function renderMoneyTools() {
  ...
  const rows = [
    ...
    ['Expensive rule', snapshot.available ? (snapshot.tooExpensive ? 'Warning only · does not block lock' : 'OK') : 'No odds'],
