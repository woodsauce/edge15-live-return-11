...
const APP_VERSION_FILE = 'edge15-genesis-hybrid-22.zip';
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const emergencyValueClean = !valueSnapshot.available || Number(valueSnapshot.winProfit || 0) >= 0.10 || Number(valueSnapshot.evPercent || 0) >= 12;
  const emergency = remainingSec >= GENESIS_MONEY_LATEST_SEC && confidence >= 90 && flipRisk <= 28 && entryQuality >= 84 && rightSide && cushion.score >= emergencyCushionRequired && genesisElite && overEarlyClean && underOpportunityClean && emergencyClean && emergencyValueClean;
...
function getTradingStyleRecommendation(decision = state.decision, candidate = null) {
  ...
  const balancedOk = isBalancedCandidate && !snapshot.tooExpensive && profit >= style.minProfit;
  return {
    styleKey,
    styleLabel: style.label,
    status: balancedOk ? 'Balanced money OK' : thinBalancedPayout ? 'Thin payout warning' : 'Wait / small payout',
    level: balancedOk ? 'good' : thinBalancedPayout ? 'warn' : 'quiet',
    message: (balancedOk
      ? `Balanced: money entry acceptable. ${payoutText}.`
      : thinBalancedPayout
        ? `Balanced: prediction may be good, but payout is thin. ${payoutText}; prefer +${formatDollar(style.minProfit)} or better on $0.50.`
        : `Balanced: official lock may still count, but payout/value is thin.`) + targetWarning,
    note: style.note
  };
}
